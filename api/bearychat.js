...
exports.newTicket = (ticket, from, to) => {
  const data = {
    text: `AobeefTicket: [#${ticket.get('nid')}](${getTicketUrl(ticket)}): ${getUserDisplayName(from)} 提交新工单`,
    attachments: [{
...
exports.replyTicket = ({ticket, reply, from, to, isCustomerServiceReply}) => {
  const data = {
    text: `AobeefTicket: [#${ticket.get('nid')}](${getTicketUrl(ticket)}): ${getUserDisplayName(from)} 回复工单`,
    attachments: [{
      title: ticket.get('title'),
...
exports.changeAssignee = (ticket, from ,to) => {
  ...
  const data = {
    text: `AobeefTicket: [#${ticket.get('nid')}](${getTicketUrl(ticket)}): ${getUserDisplayName(from)} 将工单转交给 ${getUserDisplayName(to)}`,
    attachments: [{
      title: ticket.get('title'),
      text: `该工单的问题：\n ${content} \n\n 最后一条回复：\n ${latestReply}`,
      color: COLORS.warning,
...
exports.ticketEvaluation = (ticket, from, to) => {
  ...
  const data = {
    text: `AobeefTicket: [#${ticket.get('nid')}](${getTicketUrl(ticket)}): ${getUserDisplayName(from)} 评价工单`,
    attachments: [{
      title: ticket.get('title'),
      text:
`结果：${star === 1 ? '👍'  : '👎'  }
